...
function StatusPill({ label, tone = "info" }: StatusPillProps) {
  return (
    ...
    >
      <span className="uppercase text-[10px] tracking-[0.25em] text-white/60">
        {label}
      </span>
    </span>
  );
    ...
    <header className="glass-panel relative z-[120] flex flex-col gap-4 overflow-visible rounded-2xl px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 text-sm">
        ...
        <Button
          ...
          size="sm"
          className="px-4"
          onClick={() => setCreateVestingOpen(true)}
        >
          + Create Vesting
        </Button>
        <Button
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-white/70">
            This will pause all active vesting streams. Recipients will not be
            able to claim tokens until streams are resumed.
          </p>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-white/70">
            <strong className="text-[var(--danger)]">WARNING:</strong> This will
            immediately stop and cancel all active vesting streams. This action
            cannot be undone.
          </p>
          <p className="text-sm text-white/60">
            Recipients will lose access to unvested tokens. Only use this in
            critical situations.
          </p>
